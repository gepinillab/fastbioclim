...
Una vez que tenga las variables mensuales, es hora de crear las variables bioclimáticas basadas en los promedios mensuales (n = 12 meses). Primero, necesitamos crear promedios mensuales para cada variable, y luego usar la función bioclim_terra() para obtener las 19 variables bioclimáticas.
...
```{r message=FALSE}
bios4 <- fastbioclim::bioclim_terra(
  tmin = tmin_avg, 
  tmax = tmax_avg, 
  prcp = ppt_avg,
  bios = c(5, 6, 13, 14), 
...
for (i in 1:length(mes_inicial)) {
  ...
  bios_ts <- fastbioclim::bioclim_terra(
    bios = 1:19,
    tmax = tmax_int,
    tmin = tmin_int,
    prcp = prcp_int, 
...
```
```{r}
bio13_2000_01 <- bioclim_terra(prcp = ppt[[121:360]], bios = 13)
plot(bio13_2000_01)
```
```{r}
bio13_2000_01_fix <- bioclim_terra(prcp = ppt[[121:360]], bios = 13, 
...
for (i in 1:length(mes_inicial)) {
  ...
  mois_int <-  terra::tapp(
    ...
    na.rm = TRUE) # remover NAs
  # Crear sets of variables bioclimaticas
  ts_bios_subset <- fastbioclim::bioclim_terra(
    bios = c(27, 35),
    # tmin = tmin_int,
    # tmax = tmax_int,
    srad = srad_int, 
...
wind_stats <- fastbioclim::stats_bioclim_terra(variable = wind_avg, 
...
```
```{r}
wind_stats_v2 <- fastbioclim::stats_bioclim_terra(variable = wind_avg,
                                         stats = NULL,
