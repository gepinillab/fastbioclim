...
Identifies the highest moisture of the temporal unit with the highest
value. If \`index_vector\` is \`NULL\`, it calculates the row-wise
maximum. If \`index_vector\` is provided, it extracts the value from the
...
input cells. The columns are named "bio29" (the maximum moisture) and
"cell".
