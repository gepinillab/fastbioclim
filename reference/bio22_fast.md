...
Identifies the lowest solar radiation of the temporal unit with the
lowest value. If \`index_vector\` is \`NULL\`, it calculates the
row-wise minimum. If \`index_vector\` is provided, it extracts the value
...
input cells. The columns are named "bio22" (the minimum solar radiation)
and "cell".
