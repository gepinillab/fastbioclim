...
Identifies the maximum temperature of the warmest temporal unit. If
\`index_vector\` is \`NULL\`, it calculates the row-wise maximum. If
...
- index_vector:
  ...
  \`tmax\`. Values must be between 1 and \`ncol(tmax)\`. This is
  typically used to extract the Tmax of the specific month identified as
  the warmest by another metric.

## Value

A \*\*matrix\*\* with dimensions \`c(N, 2)\`, where N is the number of
input cells. The columns are named "bio05" (the maximum temperature) and
"cell".

## Details

This function calculates bio05 following the standard definition used by
WorldClim (Hijmans et al., 2005) and ANUCLIM 6.1 (Xu & Hutchinson,
2013), which is the single highest value from all maximum temperature
layers (a "max of maxes"). It does not use mean temperature to first
